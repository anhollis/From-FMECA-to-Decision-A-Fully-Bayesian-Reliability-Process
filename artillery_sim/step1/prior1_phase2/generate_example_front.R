...
load('~/my_programs/artillery_sim/prior1_samples')

library(MCMCpack)
library(lhs)
prior_samples=prior1_samples[,c(1:25,36)]
...
save(start,file='~/my_programs/artillery_sim/prior1_phase2/start')
...
for(i in 1:length(weight_levels)){
  ga_allR(prior_samples,sys_names,sys_names[25:26],random_start,start_cw, 
          ...
          nOut = 100, nDen=512,
          directory='~/my_programs/artillery_sim/prior1_phase2/GA_res/')
}