...
#A function to compute the functional, subsystem, and system level failure probabilities from
#the failure probabilities of the base failuremodes
#failure_mode_probs: A vector of failure_mode_probabilities
...
#Computes the log-likelihood given data from a test plan and a set of failure mode failure probabilties
#data is a three column dataframe. The first column contains component names. The second contains number of failed tests for each component.
#The third contains the total number of tests for each component. 
log_lik<-function(failure_mode_probs,data){
  n<-nrow(data) #number of components
  failure_prob_table<-create_failure_table(failure_mode_probs) #Get table of all component failure probs
  log_lik=0
  for(i in 1:n){ #For each component in the data...
    prob_idx<-which(as.character(failure_prob_table$comp_name)==as.character(data$name[i]))
    p=failure_prob_table$probs[prob_idx] #get the component failure prob
    fails<-data$n_fails[i] #Determine number of failures and non-failure
    total=data$n_total[i]
    log_lik=log_lik+fails*log(p)+(total-fails)*log(1-p) #Compute log-likelihood
  }
  return(log_lik)
}

#Compute prior log density from set of failure mode failure probabilities.
log_prior<-function(failure_mode_probs){
  #Specify prior parameters for failure mode failure probabilty priors
  r1_alpha<-0.2953941
  r1_beta<-25.2289711
  r3_alpha<-1.521054
  r3_beta<-3.648332 
  ...
  betas<-c(rep(r1_beta,2),r3_beta,
           ...
           rep(r1_beta,2))
  logprior<-(alphas-1)*log(failure_mode_probs)+(betas-1)*log(1-failure_mode_probs) #Compute log prior density
  return(sum(logprior))
}

#Function to draw MH posterior draws of failure mode failure probs
mh_sampler<-function(data,init=NULL,log_prior,jump=NULL,log_lik,N=110000,burn=10000){
  init<-c(rep(0.01157302,2),0.2942427,
          ...
          rep(0.01157302,2)) #Init failure mode probabilities (MAP)
  ...
  jump<-c(rep(0.05,2),100,rep(0.05,5),
          rep(100,4),rep(0.08,2)) #Std. deviations of proposal distribution
  p<-length(init)
  acceptance_rate<-rep(0,p)
  theta_samples<-matrix(0,ncol=p,nrow=N) #Holder for posterior samples
  theta_samples[1,]=init #initialize posterior samples
  for(i in 2:N){
    if(i%%100==0){print(i)}
    #print(i)
    old_theta<-theta_samples[i-1,]
    new_theta<-rep(0,p)
    for(j in 1:p){ #Draw each failure mode failure prob individually
      prop_j<-rtruncnorm(1,a=0,b=1,mean=old_theta[j],sd=jump[j]) #Trunc norm proposal
      if(j==1){
      ...
      hastings_term=log(dtruncnorm(old_theta[j],a=0,b=1,mean=prop_j,sd=jump[j]))-log(dtruncnorm(prop_j,a=0,b=1,mean=old_theta[j],sd=jump[j])) #Correction term because of non-symmetry
      R=(log_prior(prop_all)+log_lik(prop_all,data))-(log_prior(old_theta)+log_lik(old_theta,data))+hastings_term
      U<-log(runif(1,0,1))
      if(U<R){ #Acceptance decision
        new_theta[j]=prop_j
