...
load('~/my_programs/artillery_sim/step2/prior1_phase2/posterior_samples')

library(MCMCpack)
library(lhs)
prior_samples=post_prior1_phase2[,c(1:25,36)]
#prior_samples<-system_probs(prior_samples[,1:14])
budget=80
M=100
cost=c(rep(81,14),rep(5,7),10,10,10,10,81)
constraints<-c(rep(0,14),rep(3,7),rep(3,3),1,0)
das<-randomLHS(M,26)
random_start=matrix(rep(0,M*26),ncol=26)
for(i in 15:21){
  random_start[,i]=round(qunif(das[,i],0,3))
}
for(i in 22:24){
  random_start[,i]=round(qunif(das[,i],0,3))
}

random_start[,25]=round(qunif(das[,25],0,1))
#generator<-c(rep(5,14),rep(5,7),rep(5,3),5,0)
...
save(start,file='~/my_programs/artillery_sim/step2/prior1_phase2/start')
...
for(i in 1:length(weight_levels)){
  ga_allR(prior_samples,sys_names,sys_names[25:26],random_start,start_cw, 
          ...
          nOut = 100, nDen=512,
          directory='~/my_programs/artillery_sim/step2/prior1_phase2/GA_res/')
}