library(truncnorm)
source('system_fails_from_fail_modes.R')
source('true_sim.R')
...
mh_sampler<-function(data,init=NULL,log_prior,jump=NULL,log_lik,N=110000,burn=10000){
  init<-c(rep(0.1113841,2),0.3193141,
          rep(0.1113841,5),rep(0.3193141,4),
          rep(0.1113841,2))
  log_post<-function(failure_modes,data){
    return(-log_prior(failure_modes)-log_lik(failure_modes,data))
  }
  #start<-optim(init,log_post,gr=NULL,data=data,method="L-BFGS-B",lower=rep(0.0000001,14),upper=rep(0.9999999,14))
  #init=start$par
  jump<-c(0.05,0.1,0.3,5,0.1,0.1,5,5,rep(0.3,4),0.35,20)
  p<-length(init)
